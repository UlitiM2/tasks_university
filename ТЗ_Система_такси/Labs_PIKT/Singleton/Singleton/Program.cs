...
class AudiFactory : CarFactory
{
    public override AbstractCar CreateCar()
    {
        return new AudiCar("Ауди", "белого");
    }
    public override AbstractEngine CreateEngine()
    {
...
{
    public AudiCar(string name, string col)
    {
        Name = name;
        Col = col;
    }
    ...
    public override string ToString()
    {
        return "Автомобиль " + Name + " " + Col + " цвета";
    }
}
...
class Program
{
    static void Main()
    {
        ...
        Console.WriteLine("Максимальная скорость {0} составляет {1} км/час", c3.ToString(), c3.RunMaxSpeed());
        CarFactory audi_car2 = new AudiFactory();
        Client c4 = new Client(audi_car2);
        Console.WriteLine("Максимальная скорость {0} составляет {1} км/час", c4.ToString(), c4.RunMaxSpeed());
    }
}